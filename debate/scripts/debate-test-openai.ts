...
async function main() {
  ...
  console.log("🎬 할루시네이션: 해고전쟁 — AI 토론 배틀 테스트 (OpenAI)");
  console.log("=".repeat(70));
  console.log(`\n📋 주제: ${TOPIC.title}`);
...
${collectedArguments}`;
  ...
  const costInput = (totalInputTokens / 1_000_000) * 0.15;
  const costOutput = (totalOutputTokens / 1_000_000) * 0.60;
